...
consecutiveObstaclesRight([X, Y], Ladder, Obstacles) :-
	Yone is Y+1,
	member([X, Y], Obstacles),
	(member([X, Yone], Obstacles);member([X, Yone], Ladder)).

consecutiveObstaclesLeft([X, Y], Ladder, Obstacles) :-
	Yone is Y-1,
	member([X, Y], Obstacles),
	(member([X, Yone], Obstacles);member([X, Yone], Ladder)).
	
canGoLeft([X, Y], Ladder, Obstacles, Wall) :-
	Y>0,
	\+ consecutiveObstaclesLeft([X, Y], Ladder, Obstacles),
	Yone is Y-1,
	\+(member([X, Yone], Wall)).

canGoright([X, Y], Ladder, Obstacles, Wall) :-
	Y<9,
	\+ consecutiveObstaclesRight([X, Y], Ladder, Obstacles),
	Yone is Y+1,
	\+(member([X, Yone], Wall)).
...
canGoUp([X, Y], Ladder, _, Wall) :-
	...
	% Para direita
next([X, Y], Ladder, Obstacles, Wall, [X, Yout]) :-
	canGoright([X, Y], Ladder, Obstacles, Wall),
	Yout is Y+1. 
